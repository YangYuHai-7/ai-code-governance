...
import { BUSINESS_ACCEPTANCE_RESULTS_PATH, businessConstraintRecords, readCanonicalPath, validateConfig } from '../governance/index.mjs';
...
function constraintEvidence(scan) {
  const businessAcceptanceResultsPath = readCanonicalPath(scan.root, BUSINESS_ACCEPTANCE_RESULTS_PATH);
  let config;
  try {
    config = validateConfig(readJson(path.join(scan.root, CONFIG_PATH)));
  } catch (error) {
    return { status: 'invalid', declared: 0, covered: 0, confirmedRiskSignals: [], path: businessAcceptanceResultsPath, issues: [`Cannot read current constraints: ${error.message}`] };
  }
  const confirmedRiskSignals = [...(config.confirmedRiskSignals ?? [])];
  const expected = businessConstraintRecords(config);
  if (expected.length === 0) return { status: 'not-applicable', declared: 0, covered: 0, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues: [] };
  const evidencePath = path.join(scan.root, businessAcceptanceResultsPath);
  const evidenceStat = lstatSafe(evidencePath);
  if (!evidenceStat) {
    return { status: 'missing', declared: expected.length, covered: 0, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues: [`${businessAcceptanceResultsPath}: missing owner-confirmed business acceptance evidence.`] };
  }
  let evidence;
  try {
    assertNoLinkAncestor(scan.root, businessAcceptanceResultsPath);
    if (!evidenceStat.isFile() || evidenceStat.isSymbolicLink()) throw new Error(`${businessAcceptanceResultsPath} must be a regular repository-local file.`);
    evidence = readJson(evidencePath);
  } catch (error) {
    return { status: 'invalid', declared: expected.length, covered: 0, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues: [`${businessAcceptanceResultsPath}: ${error.message}`] };
  }
  const issues = [];
  ...
  if (issues.length > 0) return { status: 'invalid', declared: expected.length, covered: 0, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues };
  ...
  if (issues.length > 0) return { status: 'invalid', declared: expected.length, covered, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues };
  ...
  if (missing.length > 0 || incomplete) {
    return {
      ...
      path: businessAcceptanceResultsPath,
      issues: missing.map((constraint) => `${constraint.id}: missing evidence bound to the current text and hash.`),
    };
  }
  return { status: 'recorded-unverified', declared: expected.length, covered, confirmedRiskSignals, path: businessAcceptanceResultsPath, issues: [] };
}
