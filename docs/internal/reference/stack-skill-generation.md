...
这些要求由 `skill-quality` 门禁执行，不是写作建议。实现型 Skill 缺少任一必需章节、非平凡的正确/错误代码块、至少三步决策流程或场景化验证矩阵时，生成直接失败。工作流型 Skill 可以不放代码，但仍必须包含触发、非触发、决策、例外、验证、项目证据边界与来源。

默认生成两个跨技术栈工作流 Skill：

- `standard-skill-authoring`：用于创建、拆分和审计标准 Skill，要求一个 Skill 对应一个稳定决策表面；
- `professional-testing`：客户提出测试时先确认是否需要模拟真人测试。若需要，先推荐质量工程师、领域专家、可用性/无障碍测试者和设备/网络覆盖人员，再推荐新用户、高频用户、边界场景用户、无障碍需求用户和目标设备/网络用户画像。没有客户明确授权，不得联系、招募、记录或补偿真实用户；模拟真人测试不得冒充真实用户验证。

技术选择读取多生态依赖事实。npm 包继续使用精确包名；Maven/Gradle 等使用精确 ecosystem + dependency 坐标。技术栈标签只能用于路由，不能单独证明某个框架规范适用。
