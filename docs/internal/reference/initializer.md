...
1. Supported Agents: Codex, Claude Code, Cursor, GitHub Copilot, generic, or a combination. Codex is the interactive default. `--clients all` selects the four built-in clients (`codex`, `claude-code`, `cursor`, `github-copilot`); config may record a selected scope, and an existing configuration is never widened implicitly. GitHub Copilot is projected to `.github/copilot-instructions.md` and `.github/skills/`; its custom agents, prompts, hooks, MCP servers and GitHub Actions stay disabled unless explicitly selected. Neither installed tools nor `--yes` decide scope.
