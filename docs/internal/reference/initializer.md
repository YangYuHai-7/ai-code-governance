...
1. Supported Agents: Codex, Claude Code, Cursor, GitHub Copilot, DeepSeek Harness, generic, or a combination. Codex is the interactive default. `--clients all` selects the four built-in clients (`codex`, `claude-code`, `cursor`, `github-copilot`); config may record a selected scope, and an existing configuration is never widened implicitly. GitHub Copilot is projected to `.github/copilot-instructions.md` and `.github/skills/`; its custom agents, prompts, hooks, MCP servers and GitHub Actions stay disabled unless explicitly selected. Neither installed tools nor `--yes` decide scope. DeepSeek Harness is selectable as `deepseek` and stays outside `--clients all`; it reads the shared `AGENTS.md`, discovers Skills under `.dsh/skills/`, and `--assist deepseek` runs `dsh --profile headless`.
...
`governanceFootprint` records the layout. A new project is `compact`: `docs/ai/` keeps only `README.md` and `context-map.yaml`, machine ledgers live in `.ai-governance/state/`, and policy, routing, evidence and integration material lives in its topic directory. An existing project is `preserve` and keeps its flat historical paths; an ordinary `sync` never moves, overwrites or deletes one. Converging a preserve tree is an explicitly approved migration: `aicg sync . --prune --dry-run` classifies every candidate as `required`, `reachable`, `dormant-managed`, or `historical-or-user`, and only the exact `--approve <planHash>` plan may migrate, archive or delete a trusted, unedited artifact. Links, drift, owner edits and unknown content are never automatic targets.

`aicg check` reports each client as `declared`, `projected`, `checked`, and `runtime-verified`. `checked` is structural consistency with the canon only and never implies a client loaded the files or obeyed them. `runtime-verified` requires a matching real probe receipt in `.ai-governance/state/client-runtime-verifications.json`, and check never launches a client to manufacture one.
...
The generated file contains owner-editable initialization input, not the final managed `.ai-governance/config.json`. It uses conservative scan-backed defaults, selects only Codex in the generated template while the visual editor defaults to the clients it detects on this machine, preserves existing code by default, and leaves ambiguous lifecycle decisions unresolved. A greenfield project starts with no technology stacks selected because there is no code to identify; an existing project starts from the stacks the scan identified and the owner confirms or corrects them. Generation is create-only: identical retries converge to `unchanged`, while different existing content, links, unsafe paths, and `.git` destinations fail closed. Validation uses the same `prepareInit` path as initialization and writes a local JSON report. The local visual editor reads and writes this same contract, offers JSON import/export, and requires a fresh exact-plan preview before applying governance. Without a path, `aicg config open` shows a project picker with recent projects, an absolute-path field, and folder browsing; an explicit path opens that project directly. After directory selection, a complete high-confidence existing-project scan presents its evidence and recorded or scan-backed choices without requiring each field again. The owner can adjust those choices, or save the suggested configuration and preview in one action. Greenfield, ambiguous, and incomplete scans keep the full choices visible; ambiguous lifecycle requires an explicit selection. The page's tool chooser offers a curated five-client order (DeepSeek, Codex, Cursor, Claude Code, GitHub Copilot) and detects what is installed locally; `generic` remains a CLI-only client. An existing managed configuration is the canonical restore source: opening a governed project reproduces the version that was actually applied, even when an editable `aicg.config.json` draft also exists. The draft is kept on disk, never consumed by the restore. If the applied configuration drifted from the recorded manifest hash, the editor reports the recorded and current hashes read-only and offers one owner-confirmed, backed-up action that regenerates config and manifest through exact approval, refreshing the recorded hashes of retained fully-managed artifacts. A repository-family root rebuilds every initialized unit under the same combined approval, backing up each unit's configuration first. An enabled Skill-management pair without an approved decision receipt is dropped by that rebuild (the generated files stay on disk), because a rebuild cannot invent an approval the configuration never carried; ordinary `init`/`sync` and `--force` continue to refuse a drifted baseline and an unapproved decision. Switching projects invalidates the previous preview. It binds to loopback and uses a per-session token and origin check. Interactive npm installation (project-local or global) attempts to open the picker automatically. `AICG_NO_AUTO_OPEN=1` disables the installation attempt.
...
The packaged public-Skill snapshot lives in `assets/registries/public-skill-sources.json` (authored pins and capability mapping) and `assets/registries/public-skill-catalog.json` (generated offline records). Setting `publicSkillCatalog: true` prepends those records to `curatedCatalog`; omitting it or setting `false` keeps the exact previous candidate set and recorded approval. Every packaged record is candidate-only, carries a pinned `verifiedAt`, and reports `refresh-due` once that pin is older than 90 days, at which point approval fails until the pin is refreshed. Maintainer commands: `npm run catalog:vendor` fetches each pinned ref into the gitignored `vendor/` cache, `npm run catalog:build` regenerates the catalog, and `npm run catalog:check` validates it offline (re-hashing vendored bytes when present). Discovering community candidates is an operator-session concern (`find-skills`, `npx skills`, skills.sh); AICG performs no network discovery or installation, and a public Skill never populates professional roles or `activation`.
...
Review every action, reason, protected path, and the exact planHash. Each retained candidate is classified `required`, `reachable`, `dormant-managed`, or `historical-or-user`. Only trusted, unchanged, fully managed artifacts may be migrated, archived or deleted inside the approved plan; unknown, seed, drifted, link, owner-edited and evidence content is retained. Plan approval binds current config/manifest, provenance, tree contents and permissions; a changed input invalidates it. `--force` or unrelated approval cannot bypass pruning. Apply/check failure restores prior files, metadata, links, and manifest transactionally.
...
Adapters are ordinary generated files or native imports: Claude Code's managed `CLAUDE.md` imports `@AGENTS.md`; Cursor's rule and GitHub Copilot's `.github/copilot-instructions.md` managed block route to the canon instead of copying it; selected client rules/Skills resolve to canonical sources. No symlink/junction adapter is created. Link migration requires explicit authorization; controlled rollback may restore the user's original link only.
