...
初始化只创建 `README.md`、`SCHEMA.md` 与空的 `INDEX.json`。AICG 可以扫描目录、调用关系和源码摘要来提出项目 Skill 候选，但不得把这些结构事实自动写成模块记忆或业务事实。只有负责人确认、且有需求、代码、测试、ADR、评审或事故证据的内容才能进入知识层。需要完整结构索引的显式 memory authoring 流程仍可调用扫描器，但这不是默认初始化行为。
