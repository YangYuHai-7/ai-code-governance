...
description: >-
  为代码仓库建立或扩展 AI 编码治理框架：生成唯一正典源、上下文路由、AGENTS.md/CLAUDE.md 适配器、skills/commands/subagents、知识记忆、机器门禁、hooks 与成长闭环。用户说“AI 编码治理框架”“AI 代码治理框架”“编码治理框架”“代码治理框架”“Agent/代理治理框架”，或在代码仓库、AGENTS.md、skills、门禁、hooks 语境下说“治理框架”时使用。未指定深度时默认进入自动完整模式；明确指定最小、标准或完整时遵从用户。不用于产品 AI 安全、模型风险、隐私合规、监管或业务治理，除非用户同时要求治理 AI 编码助手。
---
...
4. 只在不同选择会实质改变产物、需要新的外部权限，或涉及安装 git/Stop hook、修改 CI/CD、发布或外部系统时才停下询问。
5. 默认使用 `docs/ai/` 作为正典、用户当前语言作为交互与产物语言、当前 OS 作为已验证平台；其他 OS 标记 `not yet verified`。只为仓库中已检测到或用户点名的客户端生成适配器。
...
5. **规则来自本仓库的真实代码与真实事故**，不来自模板清单或另一个仓库的约定。引导模式按“读代码 → 提候选 → 用户确认 → 写入”；自动完整模式可写入有明确代码、配置、测试或 git 历史证据的规则，并标注证据。无证据的候选只记为 gap。
...
13. **技术栈包只提供候选。** 包里的架构和验证知识必须由目标仓库证据或用户选择支持，才能成为项目规则。
...
*验收*：每个 skill 的 `description` 都写清了触发条件（否则模型永远选不中它）；每个 skill、command、agent 都能从默认入口、上下文路由、能力目录或已验证的客户端原生机制找到具体路径；每条编号规则都映射到一个真实事故或代码路径。只有清单、没有加载路径的能力只能记为 `present`，不能记为 `reachable`。
