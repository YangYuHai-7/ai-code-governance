...
- **Workflow integration:** read [workflow-integrations.md](references/workflow-integrations.md), using [workflow-integration-registry.json](assets/registries/workflow-integration-registry.json). OpenSpec and explicitly selected project providers are optional. Preserve `change-authority-single-source`; claim `selective-execution-capability` only with actual runtime evidence.
