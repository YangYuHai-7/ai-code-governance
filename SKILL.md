...
For an existing project, initialization generates `docs/ai/development/index.json`, one scan-backed development document and README entrypoint per detected development unit, plus the `brownfield-understanding` Skill at Standard/Complete depth. Use that Skill to read one unit and the minimum referenced code at a time, complete responsibilities/contracts from evidence, and extract only stable reusable decision surfaces. Never convert filenames or an unexecuted command into behavioral fact.
